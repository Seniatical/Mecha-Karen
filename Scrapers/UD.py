...
def is_valid_word(name : str=None):
    if not name:
        return 'No word was specifed'
    r = requests.get('https://www.urbandictionary.com/define.php?term={}'.format(name))
    code = bs4.BeautifulSoup(r.content, 'html5lib')
...
class search:
    def __init__(self, query : str=None):
        try:
            if query.lower() == 'homepage':
                ...
                self.query = 'https://www.urbandictionary.com/' if not query else 'https://www.urbandictionary.com/define.php?term={}'.format(query.title())
        except AttributeError:
            self.query = 'https://www.urbandictionary.com/'
        self.data = requests.get(self.query)
        self.word_ = query

    def meaning(self):
        homepage = is_homepage(self.query)
        if homepage == True:
            soup = bs4.BeautifulSoup(self.data.content, 'html5lib')
        ...
        if home == True or is_valid_word(self.word_) == True:
            ...
            for i in useful().months:
                if i in full_str:
                    ...
                    data['date'] = '%s-%s-%s' % (full_str[len(full_str)-4:].strip(), useful().months.index(i, 0)+1, full_str[len(full_str)-8:len(full_str)-6])
            return data
        else:
            return {'Error 404' : 'Word not found'}
    ...
    def view(self):
        ...
        if x == False:
            ...
            holder = {}
            x = recieve(self.word_)
            word = bs4.BeautifulSoup(self.data.content, 'html5lib').find('a',attrs={'class':'word'}).text if not self.word_ else self.word_.title()
