...
from utility import boards

class games(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.running = dict()
        self.convert = commands.MemberConverter()
    ...
    async def decipher(self, ctx, opt='Easy'):
        ...
        try:
            ...
            ).set_footer(text='Well Done!', icon_url=self.bot.user.avatar))
        except asyncio.TimeoutError:
            await ctx.send(embed=discord.Embed(
                title='Failure...',
                ...
                timestamp=ctx.message.created_at
            ).set_footer(text='You lost!', icon_url=self.bot.user.avatar))
    ...
    async def roll(self, ctx, member: discord.Member = None):
        ...
        if dice < dice2:
            message = 'Looks like **{}** won.'.format(member.display_name)
            colour = discord.Colour.red()
        else:
            message = 'You win!!!'
            colour = discord.Colour.green()

        embed = discord.Embed(color=colour, description=message)
        embed.add_field(name=ctx.author.display_name, value=dice)
        embed.add_field(name=member.display_name, value=dice2)
...
⬛⬛⬛
    ...
    async def hangman(self, ctx, difficulty: str = 'Easy', multiplayer: str = ''):
        if multiplayer.lower() in ['--true', 'true']:
            def check(m):
                global letter
        ...
        global letter, attempts, guesses, participants
        start = time.time()
        ...
        attempts = 0
        display = boards[attempts]
        embed = discord.Embed(
            title='Hangman! (Game Active) %s' % multiplayer,
            colour=discord.Colour.green(),
            description='```\n' + display + '\n```',
        ...
        async def game():
            global attempts, letter, guesses, participants
            win = False
            while attempts != 9:
                try:
                    message = await self.bot.wait_for('message', check=check, timeout=30.0)
                    if multiplayer and message.author not in participants:
                ...
                if letter[0].content.lower() == 'end':
                    ...
                    embed_ = msg.embeds[0]
                    embed_.title = 'Hangman! (Game Ended) %s' % multiplayer

                    if multiplayer and participants:
                        embed_.insert_field_at(0, name='Participants', value=', '.join(map(lambda _: f'`{_.display_name}`, ', participants)))

                    embed_.colour = discord.Colour.red()
                    embed_.set_footer(text='Game lasted {:.2f} s'.format(end-start), icon_url=ctx.author.avatar)
                    ...
                    letter.pop(0)
                    embed_.colour = discord.Colour.red()
                    embed_.set_footer(text='You have already guessed this word.', icon_url=ctx.me.avatar)
                    await msg.edit(embed=embed_)
                    continue
                    ...
                    if ''.join(word_mapper) == word:
                        ...
                        embed_.title = 'Hangman! (Game Completed) %s' % multiplayer
                    ...
                    guesses.append(letter[0].content.lower())
                    embed_ = msg.embeds[0]
                    embed_.set_footer(text='I wasn\'t thinking of {}'.format(letter[0].content), icon_url=self.bot.user.avatar)
                    embed_.colour = discord.Colour.red()
            ...
            if not win:
                embed_ = msg.embeds[0]
                if multiplayer and participants:
                    embed_.insert_field_at(0, name='Participants',
                                           value=', '.join(map(lambda _: f'`{_.display_name}`, ', participants)), inline=False)
                    await msg.edit(embed=embed_)
