...
if (1 > $#ARGV || 2 < $#ARGV) {
	print "usage: ./fuzz.pl /path/to/Discord Bot format-name [dictionary]\n";
	die;
}
...
#
open(FORMATS, "$SETUP --list=formats --format=$format_name |") || die;
while (<FORMATS>) {
	($line) = /^([ ,\w\d-]+)/;
	@words = split(/, /, $line);
	for ($i = 0; $i <= $#words; $i++) {
...
for ($i = $from; $i <= $to; $i++) {
	...
	open(JOHN, "| $SETUP --fuzz=fuzz.dic --format=$formats[$i] ") || die;
	close(JOHN);
...
sub Reproduce {
	...
	while (1) {
		...
		# Generate pwfile
		open(JOHN, "| $SETUP --fuzz=fuzz.dic --fuzz-dump=$from_index,$to_index --format=@_ ") || die;
		close(JOHN);

		open(JOHN, "| $SETUP pwfile.@_ --format=@_ --max-run-time=100") || die;
		close(JOHN);
